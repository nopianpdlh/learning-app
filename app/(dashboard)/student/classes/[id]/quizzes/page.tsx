...
export default async function StudentQuizzesPage({ params }: PageProps) {
  ...
  // Get user role and student profile
  ...
  if (!profile || profile.role !== "STUDENT" || !profile.studentProfile) {
    redirect("/");
  }
  ...
  const quizzes = await prisma.quiz.findMany({
    ...
    include: {
      ...
      attempts: {
        where: { studentId: profile.studentProfile.id },
                ...
                >
                  <div className="flex items-start justify-between">
                    ...
                    <div className="ml-6">
                      {isCompleted ? (
                        <Link
                          href={`/student/classes/${classId}/quizzes/${quiz.id}/results`}
                        >
                          <Button>Lihat Hasil</Button>
                        </Link>
                      ) : isInProgress ? (
                        <Link
                          href={`/student/classes/${classId}/quizzes/${quiz.id}/take`}
                        >
