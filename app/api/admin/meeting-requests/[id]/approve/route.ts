...
export async function POST(
  ...
  try {
      ...
      if (studentProfile) {
        ...
        await db.notification.create({
          data: {
            ...
            title: "Request Jadwal Disetujui! ✅",
            message: `Request meeting Anda untuk ${
              meeting.section.template.name
            } pada ${new Date(meeting.scheduledAt).toLocaleDateString("id-ID", {
              dateStyle: "full",
            })} telah disetujui.`,
            type: "CLASS",
    ...
    await db.auditLog.create({
      data: {
        userId: user.id,
        action: "APPROVE_MEETING_REQUEST",
